...
void read_cliques(int size, char const *fname, std::vector<std::vector<std::uint32_t>> &cliques) {
    ...
    std::vector<std::uint32_t> temp_clique;
    ...
    while(size_t bytes_read = read(fd, buf, BUFFER_SIZE))
    {
        ...
        for (int i = 0; i < BUFFER_SIZE; i += 4) {
            ...
            vertex_id |= buf[i + 0] << 24;
            vertex_id |= buf[i + 1] << 16;
            vertex_id |= buf[i + 2] << 8;
            vertex_id |= buf[i + 3];

            temp_clique.push_back(vertex_id);

            if (temp_clique.size() == size) {
                        ...
                        std::vector<std::pair<std::uint32_t,std::uint32_t>> &densest_core_edge_list)
{
    ...
    std::vector<std::vector<std::vector<std::uint32_t>>> cliques;
    ...
    std::vector<std::vector<std::uint32_t>> cliques_format_temp;
    for (uint32_t i = 0; i < NUM_THREADS; i++) {
        cliques.push_back(cliques_format_temp);
    }
    delete cliques_format_temp;

    count_clique(h, g, cliques);

    int clique_count = 0;
    std::vector<uint32_t> sizes_per_thread;
    sizes_per_thread.push_back(0);

    for (uint32_t i = 0; i < NUM_THREADS; i++) {
        clique_count += cliques[i].size();
        sizes_per_thread.push_back(clique_count);
    }
